...
As the documentation is very fragmented, this is the place to put together all
the information that we have on uRISCV.


## Useful headers files

After installing the uRISV emulator, you should have the following directory:
`/usr/local/include/uriscv/`. In this directory, there are all the uRISCV headers
that can be useful. For example, the <uriscv/liburiscv.h> that implements all 
the low level calls for such as LDST, HALT, WAIT, PANIC, ecc. Is on this path.
...
back to the nucleus. Those sections summerizes all the relevant information that
...
the following:
    ...
    (1) or a trap (0). The exeption code contains the remaining information in 
    order to understand what caused the interrupt/trap. All the exemption codes 
...
4 of them are relevant for this project [4]:
    ...
    back to the exception handler in the nucleus, so the processor 
    operates with all interrupts masked. As a result, the value of MIE is set to 
...
of the `mie` register are as follows:
    ...
    Although in the Rovelli's thesis, the following table is showed [6] I don't 
...
To set the PLT, one should use the setTIMER function provided by 
...
specifications provided, however, mention the TOD clock as it's well suited for
