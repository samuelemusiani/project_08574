...
void freeMsg(msg_t *m) { list_add(&m->m_list, &msgFree_h); }
...
void mkEmptyMessageQ(struct list_head *head) { return INIT_LIST_HEAD(head); }
...
// by head. Do not remove the message from the queue. Return NULL if the queue
// is empty.
