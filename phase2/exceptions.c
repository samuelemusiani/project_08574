	...
	if (proc_was_in_user_mode((pcb_t *)BIOSDATAPAGE)) {
		...
		case SENDMESSAGE: {
			if (searchPcb(&pcbFree_h, ((pcb_t *)dest))) {
				...
				if (searchPcb(
					    &ready_queue,
					    ((pcb_t *)dest))) { // cerco nella ready queue il processo che mi interessa
					//devo modificare il registro inbox del pcb a1 (destination) con il payload a2
					insertMessage(
						&((pcb_t *)dest)->msg_inbox,
						(msg_t *)msg);

				} else if (searchPcb(
						   &unanswered_receive,
						   ((pcb_t *)dest))) { //cerco nella coda dei messaggi delle receive bloccate
					pcb_t *msg_list =
						outProcQ(&unanswered_receive,
							 ((pcb_t *)dest));
					insertMessage(&msg_list->msg_inbox,
						      (msg_t *)msg);

				} else if (searchPcb(
						   &pcb_blocked_on_clock,
						   ((pcb_t *)dest))) { //cerco un elemento nella coda dei pcb fermati per il tempo
					pcb_t *msg_list =
						outProcQ(&pcb_blocked_on_clock,
							 ((pcb_t *)dest));
					insertMessage(&msg_list->msg_inbox,
						      (msg_t *)msg);
...
static int deviceListEmpty()
{
	for (int i = 0; i < MAXDEVICE; i++) {
		if (!emptyProcQ(
			    &pcb_blocked_on_device[i])) { //problema della memcpy
			return 1;
		}
	}
	return 0;
