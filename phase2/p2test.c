...
void test()
{
	...
	STST(&p5state);
	p5Stack = p5state.reg_sp = p4state.reg_sp - (2 * QPAGE); /* because
								    there will 2
								    p4 running*/
	...
	/* create p4 */
	SYSCALL(SENDMESSAGE, (unsigned int)ssi_pcb, (unsigned int)(&p4_payload),
		0);
	SYSCALL(RECEIVEMESSAGE, (unsigned int)ssi_pcb,
		(unsigned int)(&p4_pcb_v1), 0);
	...
	SYSCALL(SENDMESSAGE, (unsigned int)ssi_pcb, (unsigned int)(&p5_payload),
		0);
	SYSCALL(RECEIVEMESSAGE, (unsigned int)ssi_pcb, (unsigned int)(&p5_pcb),
		0);
	...
	for (int i = 0; i < 2; i++) {
		...
		if (ret != DEST_NOT_EXIST)
			print_term0(
				"ERROR: second grandchild should be dead\n");
		...
		if (ret != DEST_NOT_EXIST)
			print_term0(
				"ERROR: fourth grandchild should be dead\n");
	}
...
void p2()
{
	...
	SYSCALL(SENDMESSAGE, (unsigned int)ssi_pcb,
		(unsigned int)(&get_time_payload), 0);
	SYSCALL(RECEIVEMESSAGE, (unsigned int)ssi_pcb, (unsigned int)(&cpu_t1),
		0);
	...
	SYSCALL(SENDMESSAGE, (unsigned int)ssi_pcb,
		(unsigned int)(&get_time_payload), 0);
	SYSCALL(RECEIVEMESSAGE, (unsigned int)ssi_pcb, (unsigned int)(&cpu_t2),
		0);
	STCK(now2);
	...
	if (((now2 - now1) >= (cpu_t2 - cpu_t1)) &&
		...
		if ((cpu_t2 - cpu_t1) <
		    (MINLOOPTIME / (*((cpu_t *)TIMESCALEADDR))))
...
void p4()
{
	...
	case 2:
		...
		print_term0(
			"ERROR: second incarnation of p4 didn't terminate\n");
		PANIC();
		break;
	}
...
void p5gen()
{
	unsigned int exeCode =
		pFiveSupport.sup_exceptState[GENERALEXCEPT].cause;
	switch (exeCode) {
	// store access fault
	case BUSERROR:
		print_term0(
...
void p5mm()
{
	...
	SYSCALL(RECEIVEMESSAGE, (unsigned int)ssi_pcb,
		(unsigned int)(&pFiveSupAddr), 0);

	if (pFiveSupAddr != &pFiveSupport)
		print_term0(
			"ERROR: support structure addresses are not the same\n");
	else
...
void p5sys()
{
	...
	case MSTATUS_MPP_U:
		print_term0(
			"ERROR: high level SYS call from USER mode process (should be in kernel mode)\n");
		break;
	...
	pFiveSupport.sup_exceptState[GENERALEXCEPT].pc_epc += 4; /* to avoid SYS
								    looping */
...
void p6()
{
	...
	SYSCALL(1, 0, 0, 0); /* should cause termination because p6 has no trap
				vector */
...
void p8()
{
	...
	pcb_PTR sender = (pcb_PTR)SYSCALL(RECEIVEMESSAGE,
					  (unsigned int)ANYMESSAGE, 0, 0);
...
void child1()
{
	...
	SYSCALL(SENDMESSAGE, (unsigned int)ssi_pcb,
		(unsigned int)(&get_process_payload), 0);
	SYSCALL(RECEIVEMESSAGE, (unsigned int)ssi_pcb, (unsigned int)(&pidc1),
		0);

	if (pidc1 != p8pid)
		print_term0(
			"Inconsistent (parent) process id for p8's first child\n");
...
void child2()
{
	...
	SYSCALL(SENDMESSAGE, (unsigned int)ssi_pcb,
		(unsigned int)(&get_process_payload), 0);
	SYSCALL(RECEIVEMESSAGE, (unsigned int)ssi_pcb, (unsigned int)(&pidc2),
		0);
...
void p10()
{
	...
	SYSCALL(SENDMESSAGE, (unsigned int)ssi_pcb,
		(unsigned int)(&get_process_payload), 0);
	SYSCALL(RECEIVEMESSAGE, (unsigned int)ssi_pcb, (unsigned int)(&pid10),
		0);
